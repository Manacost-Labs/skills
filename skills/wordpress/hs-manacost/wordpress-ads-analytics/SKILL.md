...
description: Protect advertising and measurement behavior for hs-manacost.ru when changing or diagnosing Playerok and other banners, ad slots, Plausible tracking, article read events, Newspaper view counters, cache invalidation, mirror delivery, or regional proxy differences. Use for stale ads, missing or duplicated analytics, wrong view counts, and cross-region inconsistencies on the primary, mirror, and staging hosts.
---

# WordPress Ads Analytics

Keep commercial placements current and measurement trustworthy without turning tests into production traffic. Pair this skill with `hs-manacost-project`, `wordpress-runtime-stack`, `wordpress-observability`, and `playwright` for browser evidence.
...
- Load one Plausible tracker per page. Keep `data-domain="hs-manacost.ru"` as the shared reporting property unless an explicitly approved analytics migration changes it.
...
- A single accepted Newspaper view action may increment `post_views_count` at most once. Cache replay, retries, the mirror rewrite, and multiple frontend handlers must not produce a second increment.
...
- Newspaper view actions and deduplication: `wordpress/mu-plugins/hs-admin-ajax-guard.php`.
...
- Runtime ad configuration in Newspaper, Ad Inserter, WP Rocket, Redis, and Cloudflare remains data/configuration; do not edit generated cache files as source.
...
python3 .agents/skills/wordpress-ads-analytics/scripts/audit_ads_analytics.py \
  --target primary=https://hs-manacost.ru/ \
  --target mirror=https://hs-manacost.com/ \
...
- Confirmation that `.com` remains functional/noindex and staging analytics remains disabled.
