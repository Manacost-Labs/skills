...
description: Safely build, change, debug, and review the hs-manacost.ru article editing experience. Use for Classic Editor, TinyMCE, Gutenberg, editor meta boxes and sidebars, autosave and revisions, article media, S3 uploads, editorial shortcodes, preview and publish flows, or changes to hs-editor-workspace and its integrations.
---

# WordPress Article Editor

Change the editorial workspace without losing drafts, silently rewriting article markup, or breaking the published result. Treat Classic Editor, TinyMCE, optional Gutenberg compatibility, Newspaper, metadata, media and custom editorial plugins as one workflow.

## Project invariants

- Work only in `/srv/projects/wordpress/hs-manacost.ru`; use Git and deploy to staging before production.
...
- Preserve Newspaper/tagDiv rendering, S3 media offload, unique filenames, article views, advertisements and cache invalidation.
- Use `wordpress-admin-ui` for every editor interface change. Also use `newspaper-tagdiv` when Newspaper, tagDiv Composer, Cloud Templates, theme CSS, blocks or modules are involved.
...
4. Choose the smallest public extension point. Prefer a scoped MU-plugin or site plugin over parent-theme, tagDiv or vendor edits.
...
- Never run blind search/replace over `post_content`, serialized metadata or tagDiv data. Any migration needs a backup, dry run, counted diff, rollback and post-migration validation.
...
- Do not modify the parent Newspaper theme or `td-*` plugins before exhausting supported extension points and running the Newspaper audit.
