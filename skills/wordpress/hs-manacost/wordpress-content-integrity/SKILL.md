...
1. Use `hs-manacost-project`, `wordpress-article-editor` and `wordpress-media-integrity` when images are involved.
...
5. Render through WordPress on `test.hs-manacost.ru`. Check desktop/mobile, anonymous/editor state and browser console/network for the actual flow.
6. Compare public `.ru` with mirror `.com`; content may match while canonical/robots must differ according to policy.
