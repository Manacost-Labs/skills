...
- Work in `/srv/projects/wordpress/kolodahearthstone.com`; treat `/var/www` as runtime only.
- Target WordPress 6.9.7, PHP-FPM 8.4, MariaDB 10.11 and the committed Blocksy integrations.
...
- Test changes on `test.kolodahearthstone.com`. Do not load-test production, enable `SAVEQUERIES`, install profilers, expose timing details, or flush caches there without explicit authorization.
