...
- Work from `/srv/projects/wordpress/hs-manacost.ru`; inspect production read-only and deploy source changes to staging first.
...
- Keep `hs-manacost.ru` canonical and indexable, `hs-manacost.com` a noindex mirror, and `test.hs-manacost.ru` noindex.
...
- If an SEO URL changes, define the canonical and one-hop redirect first, then purge affected HTML/sitemap URLs and verify on `.ru`, `.com` and staging.
