...
- State the exact commit, WordPress/PHP/Newspaper versions, environment, route, host, date, and test command.
...
## hs-manacost release minimum

- Name the single commit already deployed to `test.hs-manacost.ru`.
- Include `.ru`, `.com`, origin, Moscow, and Novosibirsk evidence for production-affecting changes.
- Confirm `.ru` canonical, `.com` noindex mirror, staging noindex, S3/media integrity, article views, banners/analytics, and targeted cache invalidation when affected.
