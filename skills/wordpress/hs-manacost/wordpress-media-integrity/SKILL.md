...
description: Audit, debug, recover, migrate, and safely change the hs-manacost WordPress image pipeline across uploads, attachment metadata, duplicate filenames, hs-local-image-optimizer, WebP/AVIF sidecars, Imagify compatibility, uploads-webpc, OVH S3 offload, Nginx negotiation, cache, mirror, staging, and regional proxies. Use for missing, wrong, overwritten, heavy, stale, or regionally unavailable images.
---
...
1. Use `hs-manacost-project`, `wordpress-article-editor` when editor insertion is involved, and `wordpress-runtime-stack` for cache/S3/proxy behavior.
...
6. Reproduce on `test.hs-manacost.ru`; keep originals byte-for-byte unchanged and generate candidates atomically.
